...
class BasePDF(FPDF):
    def header(self):
        now = datetime.now()
        if now.month < 3: semester = 'Wintersemester {0}/{1}'.format(now.year-1, now.year)
        elif now.month < 9: semester = 'Sommersemester {0}'.format(now.year)
        else: semester = 'Wintersemester {0}/{1}'.format(now.year, now.year+1)
...
class CourseGenerator(BasePDF):
    ...
    def footer(self):
        ...
        self.cell(0,7, 'Datum _________________ Unterschrift ____________________________________', 0, 1, 'R')
        self.set_font('Arial','',9)
        self.cell(0, 5, 'Personen, die nicht auf der Liste stehen, haben nicht bezahlt und sind nicht zur Kursteilnahme berechtigt. Dementsprechend können Sie auch keine Teilnahme- oder Prüfungsscheine erhalten.', 0, 1, 'C')
        self.cell(0, 5, 'Nach Kursende bitte abhaken, ob der Teilnehmer regelmäßig anwesend war, ob er die Abschlussprüfung bestanden hat und dann die unterschriebene Liste wieder zurückgeben. Danke!', 0, 1, 'C')
...
def print_bill(applicant_id, course_id):
    class BillGenerator(FPDF):
        ...
        def footer(this):
            ...
            this.cell(this.breite, 4, 'Diese Quittung wurde maschinell ausgestellt und ist ohne Unterschrift gültig.', 0, 0, 'C')
            this.cell(this.zwischenraum, 4, this.teiler, this.rahmen, 0, 'C')
            this.cell(this.breite, 4, 'Diese Quittung wurde maschinell ausgestellt und ist ohne Unterschrift gültig.', 0, 1, 'C')
...
#   fpdf.cell(w,h=0,txt='',border=0,ln=0,align='',fill=0,link='')
    ...
    resp.headers['Content-Disposition'] = 'attachment; filename="Quittung {0}.pdf"'.format(attendance.applicant.last_name)
    resp.mimetype = 'application/pdf'
